...
- [ ] **Fix NJT GTFS data refresh issue** - NJT GTFS data does not always refresh when the pm2 server restarts every 3 days. Consider alternate programmatic ways to pull new NJT GTFS data on a daily basis.
...
- [ ] **Mobile: Add Stop — default deselect all at bus terminals** — When adding a bus line at PABT (or other large bus terminals), default variant/route selection to all deselected instead of all selected, since users typically want only one variant at a time.
